      ...
      {images.length > 1 && (
        <>
          <button onClick={(e) => go(-1, e)} aria-label="Previous photo"
            className="absolute left-1.5 top-1/2 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-full cursor-pointer bg-white/85 text-gray-600 opacity-0 transition-opacity group-hover:opacity-100">
            <ChevronLeft size={15} />
          </button>
          <button onClick={(e) => go(1, e)} aria-label="Next photo"
            className="absolute right-1.5 top-1/2 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-full cursor-pointer bg-white/85 text-gray-600 opacity-0 transition-opacity group-hover:opacity-100">
            <ChevronRight size={15} />
          </button>
          <div className="absolute bottom-2 left-1/2 flex -translate-x-1/2 gap-1.5">
            {images.map((_, i) => (
              <button key={i} aria-label={`Go to photo ${i + 1}`} onClick={(e) => { e.stopPropagation(); setIndex(i); }}
                className={"h-1.5 w-1.5 rounded-full cursor-pointer transition-all " + (i === index ? "w-3.5 bg-white" : "bg-white/60")} />
            ))}
          </div>
        </>
      )}
